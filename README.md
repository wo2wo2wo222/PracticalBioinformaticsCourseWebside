...
这是课堂作业的第一部分，主要完成了课堂作业1的要求，建立了展示页面；功能页面；搜索页面，通过ajax连接到后端php再连接到数据库，进行查找数据的操作；通过管理页面，可以向数据库中插入数据；对于没有完成的页面有提示信息。
...
**  HW1的具体要求：**
  ...
  HW1暂时不需要做的内容：
  不需要建立登录界面.
  不需要利用Javascript进行页面的动态变化， 比如点击导航栏在同一页面显示内容。
  不需要对数据库查询结果进行更新或删除。
  不需要太担心数据是否收集全了。
***
